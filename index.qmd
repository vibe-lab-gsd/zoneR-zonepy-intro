...
-   May be allowed under existing zoning regulations, depending on building, parcel, or zoning characteristics that are not fully represented in the input data sets.

# Analysis workflow

Both the R and Python versions of the package analyze data stored according to the Open Zoning Feed Specification (OZFS) data standard, which is described at length by @Voulgaris2025_OZFS. The OZFS data standard includes methods for storing:
...
Both packages follow the same analysis workflow, which is illustrated in @fig-flow-chart-full and @fig-flow-chart-base. This workflow is called through a function called `zr_run_zoning_checks()` in the R version of the package and `zp_run_zoning_checks()` in the Python version. Either of these functions takes three files paths as arguments: the locations of
